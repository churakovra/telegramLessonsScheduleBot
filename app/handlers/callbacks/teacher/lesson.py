...
@router.callback_query(LessonCallback.filter(F.action == ActionType.CREATE))
async def create(
...
@router.callback_query(LessonCallback.filter(F.action == ActionType.UPDATE))
async def select_lesson(callback: CallbackQuery, session: AsyncSession) -> None:
    teacher_service = TeacherService(session)
...
@router.callback_query(LessonCallback.filter(F.action == ActionType.DELETE))
async def delete(
    callback: CallbackQuery,
    session: AsyncSession,
):
