...
async def specify_week_handler(
    ...
    markup = specify_week(
        type("Context", (), {"callback_cls": type(callback_data)})()
    )
    message = BotMessage(
...
async def create(
    ...
    message = BotMessage(
        text=BotStrings.Teacher.SLOTS_ADD,
        markup=markup
    )
    await callback.message.answer(**message.to_aiogram_kwargs())
    await callback.answer()
...
async def list_slots(
    ...
    try:
        ...
        message = BotMessage(
            text=BotStrings.Teacher.SLOTS_LIST,
            markup=markup
        )
        ...
        markup = cancel_markup(None)
        message = BotMessage(
            text=BotStrings.Common.NOT_ENOUGH_RIGHTS,
            markup=markup
        )
