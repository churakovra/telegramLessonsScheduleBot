...
async def make_teacher_from_student(message: Message, command: CommandObject):
    ...
    user_is_admin = await check_user_status(initiator_user, bv.UserRoles.ADMIN)
    teacher_is_student = await check_user_status(teacher_username, bv.UserRoles.STUDENT)
    ...
    teacher_maked = await change_user_status(initiator_user, teacher_username, bv.UserRoles.TEACHER)
